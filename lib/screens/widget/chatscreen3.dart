...
import 'package:flippra/screens/widget/vendorchat.dart';
...
class ChatScreen3 extends StatefulWidget{
  ...
  const ChatScreen3({Key? key, required this.request}) : super(key: key);
      ...
      body: SafeArea(
        child: CustomScrollView(
          slivers: [
            ...
            SliverToBoxAdapter(child: RequestVendorCard(request: widget.request,)),
            SliverToBoxAdapter(
              child: _SectionCard(
                child: SizedBox(
                  height: MediaQuery.of(context).size.height * 0.52,
                  child:  Vendorchat(request: widget.request),
                ),
              ),
            ),
          ],
...
class RequestVendorCard extends StatefulWidget {
  final ShowServiceRequestModel request;
  const RequestVendorCard({Key? key,required this.request}) : super(key: key);
...
class _RequestVendorCardState extends State<RequestVendorCard> {
  ...
  Widget build(BuildContext context) {
    final cards = List.generate(
      1,
          (i) => VendorCard(
        title: widget.request.productName,
        subtitle: "",
