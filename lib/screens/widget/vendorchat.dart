...
import '../../backend/vendor/getvendor.dart';
import '../../utils/shared_prefs_helper.dart';

class Vendorchat extends StatefulWidget {
  final ShowServiceRequestModel request;

  const Vendorchat({super.key, required this.request});
...
class _VendorchatState extends State<Vendorchat> {
  ...
  final GetChatMessageController getmessege = Get.put(GetChatMessageController());
  final ShowVendors showVendorsController = Get.put(ShowVendors());

  VendorModel? vendor; // vendor details from API
  bool _isVendorLoaded = false;
  ...
  /// 🔄 Fetch vendor data using vendor phone number from SharedPrefs
  Future<void> _loadVendor() async {
    ...
    try {
      // ✅ Step 1: Call ShowVendors API
      await showVendorsController.fetchVendors(
        token: 'wvnwivnoweifnqinqfinefnq',
        vendorPhone: phone,
      );

      if (showVendorsController.vendors.isNotEmpty) {
        vendor = showVendorsController.vendors.first;
        _isVendorLoaded = true;

        debugPrint("✅ Vendor loaded: ${vendor!.vendorEmail}");

        // ✅ Step 2: Fetch & auto-refresh chat messages
        getmessege.fetchChatMessages(
          receiverEmail: widget.request.email, // from request model
          senderEmail: vendor!.vendorEmail,
        );

        getmessege.startAutoRefresh(
          senderEmail: vendor!.vendorEmail,
          receiverEmail: widget.request.email,
        );

        if (mounted) setState(() {});
      } else {
        debugPrint("⚠️ No vendors found for this phone number");
      }
    } catch (e) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return ChatBubble(
                      ...
                      isMe: msg.senderEmail == vendor!.vendorEmail,
          ...
          Row(
            ...
            children: [
              ...
              _actionButton(context, Icons.local_shipping, "Order", () {
                ...
                (context as Element)
                    .findAncestorStateOfType<_VendorchatState>()
                    ?._showOrderDetailsBottomSheet();
              }),
            ],
          ),
...
class ChatInputBar extends StatelessWidget {
  ...
  Future<void> _sendMessage(BuildContext context) async {
    ...
    final success = await sendMessegeController.sendMessege(
      ...
      sendermail: user.vendorEmail,
      recievername: request.firstName,
      recieveremail: request.email,
